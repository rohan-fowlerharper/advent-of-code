...
type HandValue = (typeof handValueOrder)[number]
const handValueOrder = [
  'FIVE_OF_A_KIND',
  'FOUR_OF_A_KIND',
  'FULL_HOUSE',
  'THREE_OF_A_KIND',
...
const getHandType = (hand: Hand): HandValue => {
...
const sortedHands = lines
  ...
  .sort((a, b) => {
    const handTypeOrderA = handValueOrder.indexOf(a.handType)
    const handTypeOrderB = handValueOrder.indexOf(b.handType)
...
const score = sortedHands.reduce((t, h, i) => {
  return t + h.bid * (i + 1)
}, 0)

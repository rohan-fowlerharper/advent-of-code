...
// const equations: { answer: number; numbers: number[] }[] = []
...
const applyOperand = (
  a: number,
  b: number,
  operand: (typeof VALID_OPERANDS)[number]
) => {
  switch (operand) {
    case '+':
      return a + b
...
for (const eq of equations) {
  ...
  const stack: [{ runningTotal: number; numbers: number[] }] = [
    { runningTotal: numbers[0], numbers: numbers.slice(1) },
  ]
